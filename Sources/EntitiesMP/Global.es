...
// Activate class (usually touch field)           
event EActivate {};

// Deactivate class (usually touch field)
event EDeactivate {};
...
event EReceiveScore {
  INDEX iPoints,
  BOOL bEnemy, // [Cecil] Check if it's the enemy that sent the score
};
...
// Centered message printing
event ECenterMessage {
  CTString strMessage, // message
  TIME tmLength, // time on screen
  enum MessageSound mssSound, // sound to play
};

// Computer message for the player
event EComputerMessage {
  CTFileName fnmMessage, // message file
};

// Voice message for the player
event EVoiceMessage {
  CTFileName fnmMessage, // message file
};
