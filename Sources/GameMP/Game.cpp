    ...
    if (gm_bGameOn && !_pNetwork->IsPaused() && !_pNetwork->GetLocalPause()) {
      ...
      for (INDEX iPlayer = 0; iPlayer < 4; iPlayer++) {
        ...
        if (gm_lpLocalPlayers[iPlayer].lp_pplsPlayerSource != NULL) {
          ...
          memcpy(ctl_pvPlayerControls, gm_lpLocalPlayers[iPlayer].lp_ubPlayerControlsState, ctl_slPlayerControlsSize);
...
void CGame::UnpackHighScoreTable(SLONG slSize) {
  ...
  try {
    ...
    {for (INDEX i = 0; i < HIGHSCORE_COUNT; i++) {
      ...
      char strName[256];
      strLine.ScanF("\"%256[^\"]\" %d %g %d %d", strName, 
...
void CGame::Load_t(void) {
  ...
  strmFile.Read_t(&gm_MenuSplitScreenCfg, sizeof(SplitScreenCfg));
...
void CGame::Save_t(void) {
  ...
  strmFile.Write_t(&gm_MenuSplitScreenCfg, sizeof(SplitScreenCfg));
...
static void PrintStats(CDrawPort *pdpDrawPort) {
  ...
  if (hud_bShowNetGraph) {
    ...
    for (INDEX i=0; i < ctLines; i++) {
      ...
      switch (nge) {
        case NGET_ACTION:           colLine = C_GREEN;  break; // normal action (default)
        case NGET_MISSING:          colLine = C_RED;    break; // missing sequence
        case NGET_NONACTION:        colLine = C_WHITE;  break; // non-action sequence
        case NGET_REPLICATEDACTION: colLine = C_BLUE;   break; // duplicated action
        case NGET_SKIPPEDACTION:    colLine = C_YELLOW; break; // skipped action
...
void CGame::GameRedrawView(CDrawPort *pdpDrawPort, ULONG ulFlags) {
   ...
   && gm_CurrentSplitScreenCfg != SSC_DEDICATED)
  {
    ...
    BOOL bDoPrescan = _pInput->IsInputEnabled() && !_pNetwork->IsPaused()
                   && !_pNetwork->GetLocalPause() && _pShell->GetINDEX("inp_bAllowPrescan");
    ...
    // timer must not occur during prescanning
    {
      ...
      for (INDEX i = 0; i < 4; i++) {
        ...
        if (ppls != NULL) {
          ...
          if (bDoPrescan) {
            // copy its local controls to current controls
            memcpy(ctl_pvPlayerControls, gm_lpLocalPlayers[i].lp_ubPlayerControlsState, ctl_slPlayerControlsSize);
            ...
            // copy the local controls back
            memcpy(gm_lpLocalPlayers[i].lp_ubPlayerControlsState, ctl_pvPlayerControls, ctl_slPlayerControlsSize);
          }
        }
      }
    }
